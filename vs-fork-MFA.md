...
| `POST /api/auth/rotate-password` | Sets a new password and clears `must_rotate_password=false`. **Only path** that can clear the flag. | Full session + step-up within 5 min (final-Codex BLOCK 2 closure). |

## 5. Middleware (Express layer)

- **`validatedRequest`** — multi-user path requires `aud=vs-declaration` JWT, an active `user_sessions` row, `totp_verified_at != NULL`, `must_rotate_password == false` (or path is `/api/auth/rotate-password`), and that the session has not idled past 15 minutes. On idle-out the session row is revoked with `reason=idle` and the response is `401 session_expired`. On every successful pass, `last_activity_at` is touched. The touch's return value is the authoritative liveness signal — a concurrent revoke that lands between the read and the touch causes ok:false and the request is rejected with 401.
...
- **`requireFullAuth`** / **`requireFreshStepUp(maxMinutes)`** — Plan 1.5 Task 11 wires `requireFreshStepUp(5)` onto sensitive endpoints (`/api/system/export-chats`, `/api/admin/user/:id`, `/api/auth/rotate-password`, the three MFA-mutation endpoints above). `requireFullAuth` carries the same idle/touch atomicity guarantee as `validatedRequest`.
...
- **`/api/auth/rotate-password` introduced post-plan** as the final-Codex BLOCK 2 closure. The plan's `must_rotate_password` allow-list pointed at `/api/system/update-password` (single-user-mode-only — rejects in multi-user) and `/api/system/user` (too broad — would allow username/bio edits while gated). Without this endpoint, the recovery scripts' `must_rotate_password=true` permanently locked the user out of every protected route. The new endpoint requires `requireFreshStepUp(5)`; the implicit step-up granted at `/challenge` gives the operator 5 minutes from login to complete the rotation.
- **API-key endpoints (`/v1/system/export-chats`, `/v1/admin/users/:id`) are NOT decorated with `requireFreshStepUp(5)`.** Final-Codex FLAG, deliberately deferred. Rationale: API keys are issued by the COFA for their own programmatic use under the COFA-only threat model. Pairing them with TOTP step-up is architecturally tricky for headless callers and out of Plan 1.5's scope. A future Plan 1.6 ("API-key + service-account hardening") can either disable these endpoints, require an HMAC-of-step-up token, or add a per-key freshness mechanism. Recorded as a known gap, not a regression.
- **Plan 1.5 frontend (Tasks 7, 8, 8.5)** — React MFA enrol page, backup-codes confirm, MFA challenge, idle watcher, step-up modal + retry interceptor — is not yet implemented. The server enforces every spec §4.4 requirement regardless, but the AnythingLLM web UI does not yet handle the new challenge-token contract. Tracked in the plan; deferred to a separate frontend session.
